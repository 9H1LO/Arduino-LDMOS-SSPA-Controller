/* 
 *  
 *  Arduino LDMOS SSPA Controller by 9H1LO www.9h1lo.net 
 *  
...
void set_band_relay(int Nrelay) {
  if (PTT_status != HIGH ) { //need to fix - don't allow band change in MAN if PTT....if in CIV...allow
...
void fanspeed(){
 if(tempC3>20.0 && tempC3<30.0){
    analogWrite(9, 10);
   myNex.writeStr("Fs.txt", "LOW");
  }
  else if(tempC3>=30.0 && tempC3<40.0){
    analogWrite(9, 100);
    myNex.writeStr("Fs.txt", "MID");
  }
  else if(tempC3>=40.0 && tempC3<50.0){
    analogWrite(9, 155);
    myNex.writeStr("Fs.txt", "HIGH");
  }
  else if(tempC3>=50.0 && tempC3<55.0 ){
    analogWrite(9, 255);
